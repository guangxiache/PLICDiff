...
from scripts.PlicEncoder import PlicEncoder
...
class ScorePosNet3D(nn.Module):
    ...
    def __init__(self, config, protein_atom_feature_dim, ligand_atom_feature_dim):
        ...
        # pli embedding
        self.pli_encoder = PlicEncoder()
        self.pli_fusion = nn.Linear(emb_dim + self.pli_encoder.hidden_nf, emb_dim)
    ...
    def forward(self, protein_pos, protein_v, pli, batch_protein, ligand_v, ligand_pos, init_ligand_pos, init_ligand_v, batch_ligand, batch_raw_ligand,
        ...
        if h_protein.size(0) != pli.size(0):
            pli = pli.repeat(batch_size, 1)
        assert len(pli)==len(h_protein) , 'protein atom count mismatch'
        ...
        if batch_raw_ligand is not None:
            pli_emb = self.pli_encoder.extract_pli_feat(lig_coords=ligand_pos, pocket_coords=protein_pos, lig_a_hidx=ligand_v, pocket_a_hidx= gt_protein_a_h, pocket_r_hidx=gt_protein_r_h, lig_mask=batch_raw_ligand, pocket_mask=batch_protein, pli=pli,sample_diffusion=True)
        else:
            pli_emb = self.pli_encoder.extract_pli_feat(lig_coords=ligand_pos, pocket_coords=protein_pos, lig_a_hidx=ligand_v, pocket_a_hidx= gt_protein_a_h, pocket_r_hidx=gt_protein_r_h, lig_mask=batch_ligand, pocket_mask=batch_protein, pli=pli)

        h_protein_cat = torch.cat([h_protein, pli_emb],dim=-1)
        h_protein_fusion = self.pli_fusion(h_protein_cat)
        h_protein = h_protein_fusion
        ...
        return preds, info_for_aligning
    ...
    def get_diffusion_loss(
            self, protein_pos, protein_v, pli, batch_protein, ligand_pos, ligand_v, batch_ligand, time_step=None
    ):
    ...
    @torch.no_grad()
    def sample_diffusion(self, protein_pos, protein_v, pli, batch_protein,
                         init_ligand_pos, init_ligand_v, batch_ligand, batch_raw_ligand, ligand_v_reference=None, ligand_pos_reference=None,
                         num_steps=None, center_pos_mode=None, pos_only=False,
                         guidance_scale_pos=0, guidance_scale_v=0
                         ):
        ...
        ligand_pos_sampled, ligand_v_sampled = init_ligand_pos, init_ligand_v

        # time sequence
        time_seq = list(reversed(range(self.num_timesteps - num_steps, self.num_timesteps)))

        pli_uncond = torch.zeros(len(batch_protein), 7).to(protein_pos.device)
        pli_uncond[:, -1] = 1
        ...
        for i in tqdm(time_seq, desc='sampling', total=len(time_seq)):
            ...
            if pli is None or guidance_scale_pos+guidance_scale_v == 0:
                ...
                preds, _ = self(
                    ...
                    ligand_v=None,
                    ligand_pos=None,
                    init_ligand_pos=ligand_pos_sampled,
                    init_ligand_v=ligand_v_sampled,
                    batch_ligand=batch_ligand,
                    batch_raw_ligand=None,
                    time_step=t
                )
                ...
                preds_cond, _ = self(
                    ...
                    pli=pli,
                    batch_protein=batch_protein,
                    ligand_v=ligand_v_reference,
                    ligand_pos=ligand_pos_reference,
                    init_ligand_pos=ligand_pos_sampled,
                    init_ligand_v=ligand_v_sampled,
                    batch_ligand=batch_ligand,
                    batch_raw_ligand=batch_raw_ligand,
                    time_step=t
                )
                ...
                # Note: init_ligand_pos and init_ligand_v should not be confused
                # preds = {
                #     'pred_ligand_pos': preds_uncond['pred_ligand_pos'] + guidance_scale_pos * (
                #         preds_cond['pred_ligand_pos'] - preds_uncond['pred_ligand_pos']),
                #     'pred_ligand_v': preds_uncond['pred_ligand_v'] + guidance_scale_v * (
                #         preds_cond['pred_ligand_v'] - preds_uncond['pred_ligand_v'])
                # }
                preds = {
                    'pred_ligand_pos': preds_cond['pred_ligand_pos'] + guidance_scale_pos * (
                        preds_cond['pred_ligand_pos'] - preds_uncond['pred_ligand_pos']),
                    'pred_ligand_v': preds_cond['pred_ligand_v'] + guidance_scale_v * (
                        preds_cond['pred_ligand_v'] - preds_uncond['pred_ligand_v'])
                }

            # Compute posterior mean and variance
            if self.model_mean_type == 'noise':
                pred_pos_noise = preds['pred_ligand_pos'] - ligand_pos_sampled
                pos0_from_e = self._predict_x0_from_eps(xt=ligand_pos_sampled, eps=pred_pos_noise, t=t, batch=batch_ligand)
            ...
            pos_model_mean = self.q_pos_posterior(x0=pos0_from_e, xt=ligand_pos_sampled, t=t, batch=batch_ligand)
            pos_log_variance = extract(self.posterior_logvar, t, batch_ligand)
            # no noise when t == 0
            nonzero_mask = (1 - (t == 0).float())[batch_ligand].unsqueeze(-1)
            ligand_pos_next = pos_model_mean + nonzero_mask * (0.5 * pos_log_variance).exp() * torch.randn_like(
                ligand_pos_sampled)
            ligand_pos_sampled = ligand_pos_next

            if not pos_only:
                log_ligand_v_recon = F.log_softmax(v0_from_e, dim=-1)
                log_ligand_v = index_to_log_onehot(ligand_v_sampled, self.num_classes)
                ...
                ligand_v_sampled = ligand_v_next

            ori_ligand_pos = ligand_pos_sampled + offset[batch_ligand]
            pos_traj.append(ori_ligand_pos.clone().cpu())
            v_traj.append(ligand_v_sampled.clone().cpu())

        ligand_pos_sampled = ligand_pos_sampled + offset[batch_ligand]
        return {
            'pos': ligand_pos_sampled,
            'v': ligand_v_sampled,
            'pos_traj': pos_traj,
            'v_traj': v_traj,
